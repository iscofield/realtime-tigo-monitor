...
cp /path/to/nas/tigo-mqtt/data/primary/taptap.state "${BACKUP_DIR}/primary-taptap.state"
cp /path/to/nas/tigo-mqtt/data/secondary/taptap.state "${BACKUP_DIR}/secondary-taptap.state"
...
```bash
# Stop containers
sshpass -p "$PI_PASS" ssh ${PI_USER}@${PI_HOST} "sudo docker stop taptap-primary taptap-secondary"

# Restore from backup (replace TIMESTAMP with actual folder name)
BACKUP_DIR="tigo-mqtt/backups/TIMESTAMP"
cp "${BACKUP_DIR}/primary-taptap.state" /path/to/nas/tigo-mqtt/data/primary/taptap.state
cp "${BACKUP_DIR}/secondary-taptap.state" /path/to/nas/tigo-mqtt/data/secondary/taptap.state

# Restart containers
sshpass -p "$PI_PASS" ssh ${PI_USER}@${PI_HOST} "sudo docker start taptap-primary taptap-secondary"
```
