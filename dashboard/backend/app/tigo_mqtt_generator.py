...
# MQTT 3.1.1 spec allows client IDs up to 23 chars without broker accommodation.
# `taptap-` prefix consumes 7 chars, leaving 16 for the slugified name.
_CLIENT_ID_SLUG_MAX = 16


def slug(name: str, max_len: int = _CLIENT_ID_SLUG_MAX) -> str:
    """Slugify a CCA name for use in MQTT client_id (FR-2.5).

    Lowercases, replaces any non-[a-z0-9_-] character with '-', collapses
    runs of '-', strips leading/trailing '-', and truncates to max_len.
    """
...
def _client_id_for(cca: CCAConfig) -> str:
    """Build the static MQTT client ID for a CCA: `taptap-{slug(cca.name)}`."""
    return f"taptap-{slug(cca.name)}"
...
USER = ${{MQTT_USER}}
PASS = ${{MQTT_PASS}}
# Static MQTT client ID — makes broker logs searchable for this CCA.
CLIENT_ID = {client_id}
QOS = 1
TIMEOUT = 30
