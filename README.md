...
The dashboard surfaces real-time logs from your Tigo optimizers, letting you confirm
the system is working correctly and see exactly what the optimizers are sending.
This is invaluable for troubleshooting panel issues, verifying serial number mappings,
and monitoring system health — all from the browser, without SSH-ing into your data
collection device.

Two log levels are available: **info** for normal operation and **debug** for
short-term troubleshooting. Logs are persisted to disk with configurable retention.

> **Warning:** Debug logging generates substantial disk usage (~400-500 MB/panel/day
> at 5-second polling). Use it only for short-term troubleshooting.

See the [Deployment Guide](docs/DEPLOYMENT.md#logging-configuration) for log levels,
retention settings, disk usage estimates, and recommended configuration.
