...
│            └─────────────┘                                              │
│                                                                         │
│            Access: https://tigo.casadesco.com                           │
└─────────────────────────────────────────────────────────────────────────┘
```
...
```bash
curl -s "https://tigo.casadesco.com/api/panels" | python3 -c "
import sys,json
...
2. **Check if backend is receiving MQTT data:**
   ```bash
   curl -s "https://tigo.casadesco.com/api/panels" | head -c 500
   ```
...
4. **Check taptap logs for "came online" messages:**
   ```bash
   sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "sudo docker logs taptap-primary 2>&1 | grep 'came online' | wc -l"
   ```
...
**Resolution:**
```bash
sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "cd /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt && sudo docker compose down && sudo docker compose build --no-cache && sudo docker compose up -d"
```
...
```bash
sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "sudo docker rm -f taptap-primary taptap-secondary temp-id-monitor 2>/dev/null; cd /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt && sudo docker compose build && sudo docker compose up -d"
```
...
1. **Check secondary container status:**
   ```bash
   sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "sudo docker ps --filter name=taptap-secondary --format '{{.Names}}: {{.Status}}'"
   ```

2. **Check if secondary enumerated nodes:**
   ```bash
   sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "sudo docker logs taptap-secondary 2>&1 | grep 'Permanently enumerated' | wc -l"
   ```
...
3. **Check if nodes came online:**
   ```bash
   sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "sudo docker logs taptap-secondary 2>&1 | grep 'came online' | wc -l"
   ```
...
4. **Verify serial device exists:**
   ```bash
   sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "ls -la /dev/ttyACM3"
   ```

**Resolution:** If container was created before a repo restructure, rebuild:
```bash
sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "cd /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt && sudo docker compose down && sudo docker compose build && sudo docker compose up -d"
```
...
1. **Check dmesg for USB events:**
   ```bash
   sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "dmesg | grep -i 'ttyACM\|usb' | tail -20"
   ```
...
2. **Check system logs around crash time:**
   ```bash
   sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "journalctl --since '2026-01-18 10:15:00' --until '2026-01-18 10:25:00' | grep -i 'docker\|taptap' | head -30"
   ```
...
- If they don't restart, manually restart:
  ```bash
  sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "cd /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt && sudo docker compose restart"
  ```
...
1. **Check temp-id-monitor logs:**
   ```bash
   sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "sudo docker logs temp-id-monitor --tail 30"
   ```
...
2. **Check MQTT for temp_nodes:**
   ...
   sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "timeout 10 mosquitto_sub -h 192.168.2.199 -u homeassistant -P 'Ordeal!3' -t 'taptap/+/temp_nodes' -v"
   ```
...
```bash
sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "sudo docker rm -f taptap-primary taptap-secondary temp-id-monitor 2>/dev/null; cd /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt && sudo docker compose build --no-cache && sudo docker compose up -d"
```
...
| Item | Path |
|------|------|
| Project root | `/home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt` |
