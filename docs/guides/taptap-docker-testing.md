...
The taptap-mqtt containers (tigo-mqtt) **always run on the Raspberry Pi**, even during testing. This is because they require physical serial connections to the Tigo CCA devices.

For frontend/backend testing:
- Use `docker compose up --build` from the `dashboard/` directory to run locally via Docker
...
| Project Path (Pi) | /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt |
| Project Path (NAS) | /Volumes/docker/solar_tigo_viewer/tigo-mqtt |
...
```bash
# Stop all
sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "cd /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt && sudo docker compose down"

# Start all
sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "cd /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt && sudo docker compose up -d"
```

### Rebuild Images
```bash
# Rebuild after Dockerfile changes
sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "cd /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt && sudo docker compose build"

# Rebuild with no cache (force fresh download)
sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "cd /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt && sudo docker compose build --no-cache"

# Rebuild and restart
sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "cd /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt && sudo docker compose build && sudo docker compose up -d"
```
...
```bash
sshpass -p 'solar123' ssh solar-assistant@192.168.2.93 "sudo docker run --rm --entrypoint cat -v /home/solar-assistant/nas_docker/solar_tigo_viewer/tigo-mqtt/config-primary.ini:/app/config.ini:ro tigo-mqtt-taptap-primary /app/config.ini"
```
...
1. Edit config on your local machine:
   ```
   /Volumes/docker/solar_tigo_viewer/tigo-mqtt/config-primary.ini
   /Volumes/docker/solar_tigo_viewer/tigo-mqtt/config-secondary.ini
   ```
...
1. Edit Dockerfile locally:
   ```
   /Volumes/docker/solar_tigo_viewer/tigo-mqtt/Dockerfile
   ```
