...
```bash
# Stop all
ssh $PI_USER@$PI_HOST "cd /path/to/solar_tigo_viewer/tigo-mqtt && sudo docker compose down"

# Start all
ssh $PI_USER@$PI_HOST "cd /path/to/solar_tigo_viewer/tigo-mqtt && sudo docker compose up -d"
```

### Rebuild Images
```bash
# Rebuild after Dockerfile changes
ssh $PI_USER@$PI_HOST "cd /path/to/solar_tigo_viewer/tigo-mqtt && sudo docker compose build"

# Rebuild with no cache (force fresh download)
ssh $PI_USER@$PI_HOST "cd /path/to/solar_tigo_viewer/tigo-mqtt && sudo docker compose build --no-cache"

# Rebuild and restart
ssh $PI_USER@$PI_HOST "cd /path/to/solar_tigo_viewer/tigo-mqtt && sudo docker compose build && sudo docker compose up -d"
```
...
```bash
ssh $PI_USER@$PI_HOST "sudo docker run --rm --entrypoint cat -v /path/to/solar_tigo_viewer/tigo-mqtt/config-primary.ini:/app/config.ini:ro tigo-mqtt-taptap-primary /app/config.ini"
```
...
1. Edit config on your local machine:
   ```
   /path/to/solar_tigo_viewer/tigo-mqtt/config-primary.ini
   /path/to/solar_tigo_viewer/tigo-mqtt/config-secondary.ini
   ```
...
1. Edit Dockerfile locally:
   ```
   /path/to/solar_tigo_viewer/tigo-mqtt/Dockerfile
   ```
