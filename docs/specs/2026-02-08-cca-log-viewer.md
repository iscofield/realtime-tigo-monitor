...
**FR-3.6:** The backend MUST expose a test-only endpoint `POST /api/test/inject-log` for injecting log entries in environments without live CCA containers (e.g., CI, mock mode). The endpoint MUST only be available when `settings.use_mock_data` is `True`; when the flag is `False`, the endpoint MUST return `404 Not Found` (not `403`, to avoid revealing the endpoint's existence in production).
...
class LogService:
    ...
    async def ingest(self, system: str, entry: dict) -> None:
        ...
        # seq values from historical files are from previous publisher lifetimes
        # and are not contiguous with live MQTT sequences. The `if prev is not None`
        # guard below ensures the first live entry after startup passes cleanly.
    ...
    def _prune_memory(self) -> None:
        ...
        IMPORTANT: This method MUST remain synchronous (no await) to prevent
        races with ingest(). The single-threaded event loop guarantees that
        once this method starts, it runs to completion without yielding.
        ...
        for system in list(self._logs.keys()):
            ...
            if not self._logs[system]:
                ...
                self._last_seq.pop(system, None)  # Clean up stale seq tracking
        ...
        for system_dir in self.log_dir.iterdir():
            ...
            for log_file in system_dir.glob("*.log"):
                try:
                    ...
                    pass
            # Clean up empty system directories to prevent ghost entries
            # on next load_from_disk() call
            try:
...
async def inject_log(req: InjectLogRequest):
    """Test-only: inject a log entry directly (FR-3.6). Only available in mock mode."""
...
**Changes:**
- Section 2 (LogService `ingest()`): Add comment documenting why `_last_seq` is intentionally not populated by `load_from_disk()`
- Section 2 (LogService `_prune_memory()`): Add `_last_seq.pop()` cleanup when deleting empty systems to prevent stale seq tracking
- Section 2 (LogService `_prune_memory()`): Add synchronous invariant comment to prevent future races with `ingest()`
- Section 2 (LogService `ingest()`): Move `mkdir` into the existing `try/except OSError` block so filesystem errors in directory creation also degrade gracefully (memory + broadcast continue)
- Section 2 (LogService `ingest()` + `load_from_disk()`): Strengthen schema validation to check `ts` is a non-empty string and `line` is a string (prevents `TypeError` in `_prune_memory()` string comparison and silently malformed entries)
- Section 2 (LogService `_broadcast_entry()`): Add `logger.debug` when removing a WebSocket client after send error (observability improvement)
- Section 2 (LogService `load_from_disk()`): Guard `self._logs[system] = entries` with `if entries:` to prevent ghost sub-tabs from empty system directories on disk
- Section 2 (LogService `prune_old_logs()`): Add empty directory cleanup after all log files are deleted to prevent ghost entries on next restart
- FR-3.6 (new): Add test-only `POST /api/test/inject-log` endpoint specification with request/response contract, guarded by `use_mock_data` flag
- Section 4b (REST): Add inject endpoint implementation code
- Task 5 step 11: Update to reference FR-3.6 inject endpoint with concrete request example
