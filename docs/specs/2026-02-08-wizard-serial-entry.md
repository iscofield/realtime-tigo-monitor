...
**FR-1.4:** The "Next" button label SHALL read "Next: Generate Config" and SHALL be enabled when either (a) all serial fields are populated, or (b) no serials have been entered (indicating the user intends to skip).
...
**FR-2.3:** Serial number inputs SHALL accept alphanumeric characters (uppercase letters and digits). Input SHALL be auto-uppercased.
...
**FR-3.2:** The textarea SHALL accept serial numbers in CSV format (comma-separated) or TSV format (tab-separated, for spreadsheet paste). Newline-separated values SHALL also be accepted. The parser SHALL auto-detect the delimiter by checking for tabs first, then commas, then treating each line as a single value.
...
**FR-3.4:** On successful bulk import, the parsed serials SHALL immediately populate the table view above in order: filling string A positions first (A1, A2, ...), then string B positions, etc., following the string order defined in topology.
...
**FR-7.3:** If the user navigates back to the Topology step and changes string configuration (adds/removes strings, changes panel counts), the serial data SHALL be invalidated (cleared) since the panel layout has changed.
...
**NFR-1.1:** The serial entry step SHALL handle up to 100 panels per CCA without noticeable input lag.
...
**NFR-1.4:** The serial entry step state SHALL be included in the 7-day localStorage persistence. Serial data SHALL NOT exceed 50KB of storage (sufficient for hundreds of serials).
...
```typescript
function parseBulkSerials(input: string, expectedCount: number):
  ...
  if (values.length !== expectedCount) {
    return { error: `Expected ${expectedCount} serial numbers, but found ${values.length}.` };
  }

  // 4. Check for blanks
  const blankIndex = values.findIndex(v => v === '');
  if (blankIndex !== -1) {
    return { error: `Found blank entry at position ${blankIndex + 1}.` };
  }
...
The `STEP_ORDER` array and `WizardStep` type must be updated:
...
1. **Logging in `generate_tigo_mqtt_zip`:** Add a `logger.warning()` call in the `except TigoMQTTGeneratorError` branch where `generate_placeholder_ini` is invoked (this path already exists — just add the log message).

2. No API contract changes — the `GenerateConfigRequest` model already accepts optional `panels`, and the generator already handles both paths (panels present → `generate_ini_config`, panels absent → `generate_placeholder_ini`).
...
1. **Update types and wizard step definitions**
   ...
   - Add to `RESTORE_SKIP_STEPS` in `SetupWizard.tsx`
...
8. **Testing via Playwright MCP**
   ...
   - Test bulk import error cases (wrong count, blank entries)
